...
比如可以用linux自带的crontab调度能力或者xxl-job这种工具。一天同步一次，还是做不到实时或近实时。
...
如果需要处理binlog网络延迟、重复同步等异常的话，可以通过数据库唯一索引保证重复消费时的幂等性；也可以在写入数据时记录写入失败的记录稍后重试；添加数据对账机制，每天全量按主键比对保证一致性。
